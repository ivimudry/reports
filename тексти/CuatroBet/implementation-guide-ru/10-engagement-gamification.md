...
**Вход:** `total_wagered_ars` достигает следующего порога в 23 000 ARS (слоты) или 45 500 ARS (лайв)

### Правила начисления:
- 1 токен лутбокса за каждые 23 000 ARS ставок на слотах
- 1 токен лутбокса за каждые 45 500 ARS ставок в лайве
...
| Бонусные деньги | 15% | 2,300–21 000 ARS |
...
| #4 | Поставить 45 500 ARS за неделю |
...
| Полный ряд из 5 | 900 ARS + 10 FS |
| Все 30 собраны | 45 500 ARS + Premium Lucky Wheel + превью VIP I |
...
|------------|---------------------|
| Канал | Email |
| **Тема (ES)** | `🧩 900 ARS + 10 giros: te falta 1 figurita` |
| **Прехедер (ES)** | `Completá la fila y ganate el premio` |
| **Текст баннера (ES)** | `¡Te Falta 1 Figurita!` |
| **Описание баннера** | Almost-complete sticker row — 4/5 sticker slots filled, last slot glowing/pulsing with question mark, 900 ARS + 10 FS reward preview, exciting urgency |
| **Описание тела** | Подсветить, какой стикер не хватает и какое действие его открывает. Показать превью награды: 900 ARS + 10 FS. Чёткая CTA на выполнение действия. |
...
| **Описание баннера** | Celebration row complete — 5/5 stickers glowing, reward burst animation (900 ARS + 10 FS), confetti, golden completion badge |
| **Тело (ES)** | `¡Fila completa! Ganaste 900 ARS + 10 giros gratis.` |
| Действие | Начислить 900 ARS + 10 FS |
...
| **Прехедер (ES)** | `El gran premio de 45,500 ARS está cada vez más cerca` |
| **Текст баннера (ES)** | `{count} Figuritas Coleccionadas` |
| **Описание баннера** | Monthly sticker progress — sticker book grid showing filled/empty slots, progress bar to 30, grand prize preview (45,500 ARS + VIP), collector theme |
...
| 2-й | 15 FS | 75 CC | — |
| 3-й | 20 FS | 100 CC | — |
| 4-й | 30 FS | 200 CC | 50% матч до 13 500 ARS |
...
**Требуется Opt-in.** Игрок выбирает уровень: 1 500 / 3 000 / 5 000 / 45 500 ARS.
...
- [ ] Завершение ряда стикеров начисляет 900 ARS + 10 FS
