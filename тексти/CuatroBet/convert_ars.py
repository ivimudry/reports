"""
Bulk conversion: CuatroBet -> Bluechip
1. App Push -> Web Push
2. ARS values x 4.57  (single-pass, reads clean files from git)
"""
...
MULTIPLIER = 4.57
REPO = r'c:\Projects\REPORTS'
GUIDE_COMMIT = 'dcd286b'
PLAYBOOK_COMMIT = '23424c6'


def smart_round(v):
    if v < 500:   return int(round(v / 50) * 50)
    if v < 5000:  return int(round(v / 100) * 100)
    if v < 50000: return int(round(v / 500) * 500)
    if v < 500000: return int(round(v / 1000) * 1000)
...
def fmt_num(v, orig, code=False):
    v = int(v)
    if code: return str(v)
    if v < 1000: return str(v)
    sep = ',' if ',' in orig else (' ' if re.search(r'\d \d', orig) else ',')
    s = str(v)
    p = []
    while len(s) > 3:
        p.append(s[-3:]); s = s[:-3]
    p.append(s)
    return sep.join(reversed(p))


def conv(orig, code=False):
    n = parse_num(orig)
    if n is None or n == 0: return orig
    r = smart_round(n * MULTIPLIER)
    if n > 10 and n % 10 == 1: r += 1
    return fmt_num(r, orig, code)
...
def process(content):
    ...
    for old, new in [('App Push','Web Push'),('APP PUSH','WEB PUSH'),('app push','web push')]:
        ...
        if c:
            ...
            log.append(f"  '{old}' -> '{new}' ({c}x)")

    # 2. Single-pass master pattern: range | slash | single
    mp = re.compile(
        r'(?<![.\d\w])(?:'
        r'(' + _N + r')([ ]*[-\u2013][ ]*)(' + _N + r')([ ]*ARS)\b'
        r'|(\d[\d,]*(?:[ ]*/[ ]*\d[\d,]*)+)([ ]*ARS)\b'
    ...
    def rpl(m):
        if m.group(1) is not None:
            a, s, b, x = m.group(1), m.group(2), m.group(3), m.group(4)
            ca, cb = conv(a), conv(b)
            log.append(f"  Range: {a}{s.strip()}{b} ARS -> {ca}{s.strip()}{cb} ARS")
            return f"{ca}{s}{cb}{x}"
        elif m.group(5) is not None:
            ns, x = m.group(5), m.group(6)
            ps = re.split(r'[ ]*/[ ]*', ns)
            nps = [conv(p) for p in ps]
            log.append(f"  Slash: {ns} ARS -> {'/'.join(nps)} ARS")
            return '/'.join(nps) + x
        else:
            n, x = m.group(7), m.group(8)
            c = conv(n)
            if c != n: log.append(f"  Single: {n} ARS -> {c} ARS")
            return f"{c}{x}"

    content = mp.sub(rpl, content)

    # 3. Code: _ars OP number  (NOT followed by ARS)
    cp = re.compile(r'(_ars\s*(?:>=|<=|\u2265|\u2264|>|<|=)\s*)(\d[\d,]*)(?!\s*ARS)', re.I)
    def rc(m):
        p, n = m.group(1), m.group(2)
        c = conv(n, code=True)
        if c != n: log.append(f"  Code: {p.strip()}{n} -> {p.strip()}{c}")
        return f"{p}{c}"
    content = cp.sub(rc, content)

    # 4. Code: _ars BETWEEN X AND Y
    bp = re.compile(r'(_ars\s+BETWEEN\s+)(\d+)(\s+AND\s+)(\d+)', re.I)
    def rb(m):
        p, a, mid, b = m.group(1), m.group(2), m.group(3), m.group(4)
        ca, cb = conv(a, True), conv(b, True)
        log.append(f"  BETWEEN: {a} AND {b} -> {ca} AND {cb}")
        return f"{p}{ca}{mid}{cb}"
    content = bp.sub(rb, content)

    # 5. Code: number - cumulative_deposits_ars
    ap = re.compile(r'(?<![.\d\w])(\d+)(\s*[-\u2013]\s*cumulative_deposits_ars)')
    def ra(m):
        n, s = m.group(1), m.group(2)
        c = conv(n, True)
        if c != n: log.append(f"  Before attr: {n} -> {c}")
        return f"{c}{s}"
    content = ap.sub(ra, content)

    return content, log


def git_show(commit, rel_path):
    r = subprocess.run(
        ['git', 'show', f'{commit}:{rel_path.replace(os.sep, "/")}'],
        capture_output=True, text=True, encoding='utf-8', cwd=REPO)
    return r.stdout if r.returncode == 0 else None


# -- Note to inject into doc 02 --
NOTE_OLD = (
    '**\u0422\u0440\u0438\u0433\u0433\u0435\u0440 \u0432\u0445\u043e\u0434\u0430:** '
    '`cumulative_deposits_ars >= 150000` AND `days_since_last_activity <= 14`  \n'
    '**\u041f\u043e\u0432\u0442\u043e\u0440\u043d\u044b\u0439 \u0432\u0445\u043e\u0434:** '
    '\u041d\u0435\u0442  \n'
    '**\u041e\u043a\u043d\u043e Journey:** 30 \u0434\u043d\u0435\u0439'
)
NOTE_NEW = (
    '**\u0422\u0440\u0438\u0433\u0433\u0435\u0440 \u0432\u0445\u043e\u0434\u0430:** '
    '`cumulative_deposits_ars >= 150000` AND `days_since_last_activity <= 14`  \n'
    ...
    '\u041d\u0435\u0442\n\n'
    '> **\u041f\u0440\u0438\u043c\u0435\u0447\u0430\u043d\u0438\u0435:** '
    '\u0418\u0433\u0440\u043e\u043a\u0438 \u0441 150,000\u2013200,000 ARS '
    '\u043f\u043e\u043b\u0443\u0447\u0430\u044e\u0442 \u043e\u0444\u0444\u0435\u0440\u044b '
    '\u043f\u043e \u043c\u043d\u043e\u0436\u0438\u0442\u0435\u043b\u044e High (\u00d72) '
    '\u0434\u043e \u0434\u043e\u0441\u0442\u0438\u0436\u0435\u043d\u0438\u044f 200,000 ARS, '
    '\u0437\u0430\u0442\u0435\u043c \u043f\u0435\u0440\u0435\u0445\u043e\u0434\u044f\u0442 '
    '\u043d\u0430 Pre-VIP+ (\u00d73).\n\n'
    '**\u041e\u043a\u043d\u043e Journey:** 30 \u0434\u043d\u0435\u0439'
)
...
gdir = os.path.join(base, 'implementation-guide-ru')

files = [(f, GUIDE_COMMIT) for f in sorted(glob.glob(os.path.join(gdir, '*.md')))]
files.append((os.path.join(base, 'cuatrobet_playbook_ru.md'), PLAYBOOK_COMMIT))
files.append((os.path.join(base, 'cuatrobet_playbook_en.md'), PLAYBOOK_COMMIT))

total = 0
for fp, commit in files:
    rel = os.path.relpath(fp, REPO)
    orig = git_show(commit, rel)
    if orig is None:
        print(f"  SKIP: {os.path.basename(fp)}")
        continue

    # Inject note for doc 02
    if '02-pre-vip-lifecycle' in fp:
        orig = orig.replace(NOTE_OLD, NOTE_NEW)

    out, log = process(orig)
